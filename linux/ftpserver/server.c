...
void user_pwd(int);
void user_ls(int);
void user_cd(int,char *);
void user_rmdir(int,char *);
void user_login(int,char *,char *,int);
void user_mul_get(int,int,char *);
void user_mul_put(int,int,char *);
        ...
        while(read(sockfd, recv_commod, N)>0){
            ...
            if(!strncmp(recv_commod,"login", 5)){
                ...
                user_ls(sockfd);
                    ...
                    while(strtok(NULL," "))
                        file_nums++;
                    user_mul_get(sockfd,file_nums,commds);
                ...
                if(!strstr(user_rights,"W")){
                    ...
                    while(strtok(NULL," "))
                        file_nums++;
                    user_mul_put(sockfd,file_nums,commds);
                }
...
上传单个/多个文件（put/mput）、下载单个/多个文件（get/mget）。
*/
void user_mkdir(int sockfd,char * path_name){
    char dir[N];
...
void user_rmdir(int sockfd,char *path_name){
...
void user_cd(int sockfd,char * path_name){
    char dir[N];
    getcwd(dir, sizeof(dir));
    //将路径完整
    if(!strncmp(path_name,"/",1)){
...
//用于下载单个多个文件
void user_mul_get(int sockfd,int file_nums,char *commd){
    int fd;
    int nbytes;
    char buffer[N];
    char file_name[N];
...
//用于上传文件
void user_mul_put(int sockfd,int file_nums,char *commd){
